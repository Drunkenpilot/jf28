...
<a class="btn btn-primary btn-sm" href="<?=site_url('catalog/categories/addNewCategory')?>"><i class="glyphicon glyphicon-plus-sign"></i> <?=nbs(1)?> new category</a>
</div>
<div class="table-responsive" >
	<table class="table table-hover">
		...
		<tbody>
		<?php  $last_position = count($topLevel->children);?>
		<?php foreach ($topLevel->children as $cat) :?>
			...
			<form actions="" method="post" role="form" class="form-inline" id="position_down_<?=$cat->id?>" enctype="multipart/form-data">
				...
				<td>				
				<button type="button" class="btn btn-primary btn-xs" onclick="position_up(<?=$cat->id?>);" <?php if($cat->position==1){?>disabled<?php }?>><i class="glyphicon glyphicon-circle-arrow-up"></i></button> 
				<?=$cat->position?> 
				<button type="button" class="btn btn-primary btn-xs" onclick="position_down(<?=$cat->id?>);" <?php if($last_position == $cat->position){?>disabled<?php }?>><i class="glyphicon glyphicon-circle-arrow-down"></i></button>
				</td>		
